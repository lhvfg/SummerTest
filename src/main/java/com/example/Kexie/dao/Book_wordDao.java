...
public interface Book_wordDao extends BaseMapper<Book_word> {
    ...
    Integer getLearnNum(Integer bookId,Integer userId);
    // 查询要复习的单词数量
    @Select("SELECT count(*) from book_word where book_id = #{bookId} and word_id in (SELECT word_id from word_user where recite = 1 and finish = 0 and user_id = #{userId})")
    Integer getReviewNum(Integer bookId,Integer userId);
}
